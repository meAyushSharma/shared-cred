    ...
    if (!username || !password || !name) {
      ...
      try {
          User.create(user).then((user) => {
            const token = jwt.sign({username: user.username, password: user.password, name: user.name,},process.env.JWT_SECRET_KEY);
            ...
            throw new ExpressError(`this is the error while creating user : ${err}`, 500);
        }
}
...
module.exports.loginUser = async (req, res) => {
    ...
    User.findOne({username: username})
      .then(async (ifUserExist) => {
        ...
        const token = jwt.sign({ username: ifUserExist.username, password: ifUserExist.password, name: ifUserExist.name }, process.env.JWT_SECRET_KEY);
        res.cookie("token", token, {
            maxAge: 3 * 60 * 60 * 1000,
            httpOnly: true,
            secure: true,
